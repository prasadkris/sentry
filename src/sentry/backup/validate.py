...
def validate(
    ...
    class OrdinalCounter:
        ...
        max_seen_pk: int
        max_unused_ordinal: int

        def __init__(self):
            self.max_seen_pk = -1
            self.max_unused_ordinal = 1
        ...
        def assign(self, obj: JSONData, side: Side) -> Tuple[int, list[ComparatorFinding]]:
            ...
            if pk > self.max_seen_pk:
                ...
                findings.append(
                    ComparatorFinding(
                        kind="UnorderedInput",
                        on=InstanceID(model, self.max_unused_ordinal),
            ...
            obj["ordinal"] = self.max_unused_ordinal
            self.max_unused_ordinal += 1
    ...
    for model_name in model_names:
        left_count = left_ordinal_counters.get(model_name, OrdinalCounter()).max_unused_ordinal
        right_count = right_ordinal_counters.get(model_name, OrdinalCounter()).max_unused_ordinal
